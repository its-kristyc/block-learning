...
export function Editor({ draft, setDraft, onSave, onCancel, openFrom, isMobile }) {
  ...
  // "Warm Up" (block 1) has no exercises stored under it — it's the 4 warm-up
  // series (Mat Foundation/Intermediate, Cadillac, F2 Chair). Resolve the union
  // of their exercise IDs so the filter surfaces them (name-based placeholders
  // that don't resolve to a real exercise are dropped).
  const warmupIds = useMemo(() => new Set(
    WARMUP_SERIES.flatMap(s => s.exercises).filter(ref => byId[ref])
  ), []);
  const blockOptions = BLOCK_DISPLAY_ORDER.map(n => BLOCKS[n - 1]);
  const matchesBlock = e => !libFilters.block.length || libFilters.block.some(b =>
    b === 'Warm Up' ? warmupIds.has(e.id) : BLOCKS[e.block - 1] === b
  );
      ...
      <div style={{ paddingBottom: 90 }}>
        ...
        <div style={{ padding: '4px 14px', display: 'flex', flexDirection: 'column', gap: 10 }}>
          {BLOCKS.slice(0, 12).map((name, i) => {
            const n = i + 1;
            const displayName = n === 1 ? 'Warm Up / Foundation' : name;
            ...
            return (
              <div key={n} style={{ background: C.card, border: `1px solid ${C.line}`, borderRadius: 12, padding: '10px 12px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: ids.length ? 8 : 0 }}>
                  ...
                  <span style={{ fontSize: 13, fontWeight: 800, textTransform: 'uppercase', letterSpacing: 0.5, flex: 1, color: C.muted }}>{displayName}</span>
                  <button
            ...
            return (
              ...
              >
                <div style={{ display: 'flex', gap: 6, alignItems: 'baseline', marginBottom: 8 }}>
                  ...
                  <span style={{ fontSize: 12, fontWeight: 800, textTransform: 'uppercase', letterSpacing: 0.4, lineHeight: 1.25, color: C.muted }}>{displayName}</span>
                </div>
